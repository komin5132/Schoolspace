...
class ManageUsersFragment : Fragment(R.layout.fragment_manage_users) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        adapter = UserAdapter(userList) { user -> showEditUserDialog(user) }
    ...
    private fun showEditUserDialog(user: User) {
        ...
        // Wychowawcą może być tylko nauczyciel
        cbIsTutor.visibility = if (user.role == "teacher") View.VISIBLE else View.GONE
        
        spinnerRole.onItemSelectedListener = object : AdapterView.OnItemSelectedListener {
            override fun onItemSelected(parent: AdapterView<*>?, view: View?, position: Int, id: Long) {
                cbIsTutor.visibility = if (roles[position] == "teacher") View.VISIBLE else View.GONE
            }
    ...
    class UserAdapter(private val users: List<User>, private val onEdit: (User) -> Unit) : RecyclerView.Adapter<UserAdapter.ViewHolder>() {
        ...
        override fun onBindViewHolder(holder: ViewHolder, position: Int) {
            ...
            holder.details.text = "${user.role}$tutorInfo | Klasa: ${if(user.className.isEmpty()) "Brak" else user.className}"
